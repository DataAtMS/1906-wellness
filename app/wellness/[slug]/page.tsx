...
import { getWellnessArticles, getArticleBySlug, formatDate } from "@/lib/articles";
import { renderMarkdown } from "@/lib/markdown";
import { WELLNESS_TAGLINE } from "@/lib/taglines";

export async function generateStaticParams() {
  return getWellnessArticles().map((a) => ({ slug: a.slug }));
}
...
export async function generateMetadata({
  ...
  const a = getArticleBySlug("wellness", slug);
...
export default async function WellnessArticlePage({
  params,
}: {
  params: Promise<{ slug: string }>;
}) {
  ...
  return (
    <article className="max-w-3xl mx-auto px-4 sm:px-6 py-10">
      ...
      <p className="text-xs uppercase tracking-widest text-brand-muted mb-3">
        Health and Wellness · {a.category}
      </p>
      ...
      <p className="mt-12 pt-6 border-t border-brand-border">
        <Link href="/wellness" className="text-sm underline">
